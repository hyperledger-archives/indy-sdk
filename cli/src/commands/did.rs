...
pub mod new_command {
    ...
    fn execute(ctx: &CommandContext, params: &CommandParams) -> Result<(), ()> {
        ...
        let res = match res {
            ...
            Err(ErrorCode::CommonInvalidStructure) => Err(println_err!("Invalid format of command params. Please check format of posted json's, keys or did's and etc...")),
...
pub mod rotate_key_command {
    ...
    fn execute(ctx: &CommandContext, params: &CommandParams) -> Result<(), ()> {
        ...
        let new_verkey = match Did::replace_keys_start(wallet_handle, &did, &identity_json) {
            ...
            Err(_) => return Err(println_err!("Invalid format of command params. Please check format of posted json's, keys or did's and etc...")),
        }?;
