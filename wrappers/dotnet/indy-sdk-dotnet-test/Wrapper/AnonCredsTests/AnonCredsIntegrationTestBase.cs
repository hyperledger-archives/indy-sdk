using Indy.Sdk.Dotnet.Wrapper;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using System;
using System.Threading.Tasks;
using static Indy.Sdk.Dotnet.Wrapper.Agent;
using static Indy.Sdk.Dotnet.Wrapper.AgentObservers;

namespace Indy.Sdk.Dotnet.Test.Wrapper.AnonCredsTests
{

    public abstract class AnonCredsIntegrationTestBase 
    {
        //protected static Wallet _wallet;
        //protected static string _claimDef;

        //private static bool _walletOpened = false;

        //private string _masterSecretName = "master_secret_name";
        //private string _issuerDid = "NcYxiDXkpYi6ov5FcYDi1e";
        //private string _issuerDid2 = "CnEDk9HrMnmiHXEV1WFgbVCRteYnPqsJwrTdcZaNhFVW";
        //private string _proverDid = "CnEDk9HrMnmiHXEV1WFgbVCRteYnPqsJwrTdcZaNhFVW";
        //private string _claimOfferTemplate = "{{\"issuer_did\":\"{0}\",\"schema_seq_no\":{1}}}";
        //private string _schema = "{\"seqNo\":1,\"data\": {\"name\":\"gvt\",\"version\":\"1.0\",\"keys\":[\"age\",\"sex\",\"height\",\"name\"]}}";
        //private string _claimRequestTemplate = 
        //    "{\"blinded_ms\":" +
        //    "{\"prover_did\":\"CnEDk9HrMnmiHXEV1WFgbVCRteYnPqsJwrTdcZaNhFVW\"," +
        //    "\"u\":\"\"," +
        //    "\"ur\":null}," +
        //    "\"issuer_did\":\"%s\",\"schema_seq_no\":%d}";

        [TestInitialize]
        public void SetUp() 
        {
            InitHelper.Init();
            StorageUtils.CleanupStorage();
        }

      //  protected void InitCommonWallet()
      //  {

		    //if (_walletOpened) {
      //          return;
      //      }

      //      StorageUtils.CleanupStorage();

      //      var walletName = "anoncredsCommonWallet";

      //      Wallet.CreateWalletAsync("default", walletName, "default", null, null).Wait();
      //      _wallet = Wallet.OpenWalletAsync(walletName, null, null).Result;

      //      _claimDef = AnonCreds.IssuerCreateAndStoreClaimDefAsync(_wallet, _issuerDid, _schema, null, false).Result;

      //      AnonCreds.ProverStoreClaimOfferAsync(_wallet, string.Format(_claimOfferTemplate, _issuerDid, 1)).Wait();
      //      AnonCreds.ProverStoreClaimOfferAsync(_wallet, string.Format(_claimOfferTemplate, _issuerDid, 2)).Wait();
      //      AnonCreds.ProverStoreClaimOfferAsync(_wallet, string.Format(_claimOfferTemplate, _issuerDid2, 2)).Wait();

      //      AnonCreds.ProverCreateMasterSecretAsync(_wallet, _masterSecretName).Wait();

      //      var claimOffer = string.Format("{{\"issuer_did\":\"{0}\",\"schema_seq_no\":{1}}}", _issuerDid, 1);

      //      var claimRequest = AnonCreds.ProverCreateAndStoreClaimReqAsync(_wallet, "CnEDk9HrMnmiHXEV1WFgbVCRteYnPqsJwrTdcZaNhFVW", claimOffer, _claimDef, _masterSecretName).Result;

      //      var claim = "{\"sex\":[\"male\",\"5944657099558967239210949258394887428692050081607692519917050011144233115103\"],\n" +
      //              "                 \"name\":[\"Alex\",\"1139481716457488690172217916278103335\"],\n" +
      //              "                 \"height\":[\"175\",\"175\"],\n" +
      //              "                 \"age\":[\"28\",\"28\"]\n" +
      //              "        }";

      //      var createClaimResult = AnonCreds.IssuerCreateClaimAsync(_wallet, claimRequest, claim, -1, -1).Result;
      //      var claimJson = createClaimResult.ClaimJson;

      //      AnonCreds.ProverStoreClaimAsync(_wallet, claimJson).Wait();

      //      _walletOpened = true;
      //  }
    }
}
