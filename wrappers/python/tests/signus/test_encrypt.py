...
@pytest.mark.asyncio
async def test_encrypt_works_for_pk_cached_in_wallet(pool_handle, wallet_handle, identity_my1, identity_trustee1):
...
async def test_encrypt_works_for_expired_nym(wallet_handle, pool_handle, identity_trustee1, identity_my1, wallet_runtime_config):
    (trustee_did, _) = identity_trustee1
    (my_did, my_verkey) = identity_my1
...
@pytest.mark.asyncio
async def test_encrypt_works_for_invalid_wallet_handle(wallet_handle, pool_handle, identity_trustee1, identity_my1):
    (trustee_did, _) = identity_trustee1
    (my_did, my_verkey) = identity_my1

    with pytest.raises(IndyError) as e:
        await signus.store_their_did(wallet_handle, json.dumps({"did": my_did, "verkey": my_verkey}))
        await signus.encrypt(wallet_handle + 1, pool_handle, trustee_did, my_did, message)
    assert ErrorCode.WalletInvalidHandle == e.value.error_code


@pytest.mark.asyncio
async def test_encrypt_works_for_invalid_pool_handle(wallet_handle, pool_handle, identity_trustee1, identity_my1):
    (trustee_did, _) = identity_trustee1
    (my_did, my_verkey) = identity_my1

    with pytest.raises(IndyError) as e:
        await signus.store_their_did(wallet_handle, json.dumps({"did": my_did, "verkey": my_verkey}))
        await signus.encrypt(wallet_handle, pool_handle + 1, trustee_did, my_did, message)
