...
async def test_set_key_metadata_works(wallet_handle, verkey_my1, metadata):
    await signus.set_key_metadata(wallet_handle, verkey_my1, metadata)


@pytest.mark.asyncio
async def test_set_key_metadata_works_for_replace(wallet_handle, verkey_my1, metadata):
    await signus.set_key_metadata(wallet_handle, verkey_my1, metadata)
    received_metadata = await signus.get_key_metadata(wallet_handle, verkey_my1)
    assert metadata == received_metadata

    new_metadata = 'new metadata'
    await signus.set_key_metadata(wallet_handle, verkey_my1, new_metadata)
    updated_metadata = await signus.get_key_metadata(wallet_handle, verkey_my1)
    assert new_metadata == updated_metadata


@pytest.mark.asyncio
async def test_set_key_metadata_works_for_empty_string(wallet_handle, verkey_my1):
    await signus.set_key_metadata(wallet_handle, verkey_my1, '')
...
async def test_set_key_metadata_works_for_invalid_handle(wallet_handle, verkey_my1, metadata):
    with pytest.raises(IndyError) as e:
        ...
        await signus.set_key_metadata(invalid_wallet_handle, verkey_my1, metadata)
