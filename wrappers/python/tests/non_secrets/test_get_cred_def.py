...
async def test_get_cred_def_works(pool_handle, wallet_handle, identity_my):
    ...
    schema_json = await non_secrets.get_schema(pool_handle, wallet_handle, my_did, schema_id, json.dumps(options_json))
    ...
    await non_secrets.get_cred_def(pool_handle, wallet_handle, my_did, cred_def_id, json.dumps(options_json))
