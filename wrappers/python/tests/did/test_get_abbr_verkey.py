...
@pytest.mark.asyncio
async def test_get_abbr_verkey_for_abbbr_key(wallet_handle):
    (_did, full_verkey) = await did.create_and_store_my_did(wallet_handle, "{}")
    verkey = await did.get_abbr_verkey(_did, full_verkey)
    assert not full_verkey == verkey


@pytest.mark.asyncio
async def test_get_abbr_verkey_for_abbbr_key(wallet_handle, did_my1):
