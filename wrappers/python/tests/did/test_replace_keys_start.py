...
async def test_replace_keys_start_works(wallet_handle):
    ...
    new_ver_key = await did.replace_keys_start(wallet_handle, did, "{}")
...
async def test_replace_keys_start_works_for_seed(wallet_handle, seed_my1, verkey_my1):
    (did, ver_key) = await did.create_and_store_my_did(wallet_handle, "{}")
    new_ver_key = await did.replace_keys_start(wallet_handle, did, json.dumps({'seed': seed_my1}))
    assert new_ver_key != did
...
async def test_replace_keys_start_works_for_correct_crypto_type(wallet_handle, crypto_type):
    (did, ver_key) = await did.create_and_store_my_did(wallet_handle, "{}")
    new_ver_key = await did.replace_keys_start(wallet_handle, did, json.dumps({"crypto_type": crypto_type}))
    assert new_ver_key != did
...
async def test_replace_keys_start_works_for_invalid_handle(wallet_handle):
    with pytest.raises(IndyError) as e:
        ...
        await did.replace_keys_start(wallet_handle + 1, did, "{}")
...
async def test_replace_keys_works_start_for_invalid_crypto_type(wallet_handle):
    with pytest.raises(IndyError) as e:
        (did, _) = await did.create_and_store_my_did(wallet_handle, "{}")
        await did.replace_keys_start(wallet_handle, did, '{"crypto_type": "type"}')
