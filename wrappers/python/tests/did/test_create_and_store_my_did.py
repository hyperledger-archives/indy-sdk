...
async def test_create_my_did_works_for_seed(wallet_handle, seed_my1, did_my1, verkey_my1):
    (did, ver_key) = await did.create_and_store_my_did(wallet_handle, json.dumps({'seed': seed_my1}))
    assert did_my1 == did
...
async def test_create_my_did_works_as_cid(wallet_handle, seed_my1, verkey_my1):
    (did, ver_key) = await did.create_and_store_my_did(wallet_handle, json.dumps({'seed': seed_my1, 'cid': True}))
...
async def test_create_my_did_works_for_passed_did(wallet_handle, did_my1):
    (did, _) = await did.create_and_store_my_did(wallet_handle, json.dumps({'did': did_my1}))
    assert did_my1 == did
