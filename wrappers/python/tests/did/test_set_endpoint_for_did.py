...
async def test_set_endpoint_for_did_works(wallet_handle, identity_trustee1, endpoint):
    ...
    await did.set_endpoint_for_did(wallet_handle, did, endpoint, verkey)
...
async def test_set_endpoint_for_did_works_for_replace(pool_handle,wallet_handle, identity_trustee1, endpoint):
    ...
    await did.set_endpoint_for_did(wallet_handle, did, endpoint, key)
    received_endpoint, received_key = await did.get_endpoint_for_did(wallet_handle, pool_handle, did)
    ...
    await did.set_endpoint_for_did(wallet_handle, did, new_endpoint, new_key)
    updated_endpoint, updated_key = await did.get_endpoint_for_did(wallet_handle, pool_handle, did)
...
async def test_set_endpoint_for_did_works_for_invalid_transport_key(wallet_handle, identity_trustee1, endpoint):
    ...
    with pytest.raises(IndyError) as e:
        await did.set_endpoint_for_did(wallet_handle, did, endpoint, 'CnEDk___MnmiHXEV1WFgbV___eYnPqs___TdcZaNhFVW')
    assert ErrorCode.CommonInvalidStructure == e.value.error_code
...
async def test_set_endpoint_for_did_works_for_invalid_handle(wallet_handle, identity_trustee1, endpoint):
    ...
    with pytest.raises(IndyError) as e:
        ...
        await did.set_endpoint_for_did(invalid_wallet_handle, did, endpoint, key)
