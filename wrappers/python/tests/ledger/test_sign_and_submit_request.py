...
async def test_sign_and_submit_request_works_for_incompatible_wallet_and_pool(pool_name, pool_handle, wallet_name,
    ...
    (my_did, _, _) = await signus.create_and_store_my_did(wallet_handle, json.dumps({"seed": seed_my1}))
    (trustee_did, _, _) = await signus.create_and_store_my_did(wallet_handle, json.dumps({"seed": seed_trustee1}))
