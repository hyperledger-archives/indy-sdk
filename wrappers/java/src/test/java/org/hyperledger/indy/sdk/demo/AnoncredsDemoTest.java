...
public class AnoncredsDemoTest extends IndyIntegrationTest {
	...
	public void anoncredsWorksForMultiplyIssuerSingleProver() throws Exception {
		...
		//2. Issuer1 Create and Open Wallet
		Wallet.createWallet(poolName, "issuer1Wallet", "default", null, null).get();
		Wallet issuerGvtWallet = Wallet.openWallet("issuer1Wallet", null, null).get();

		//3. Issuer2 Create and Open Wallet
		...
		//4. Prover Create and Open Wallet
		Wallet.createWallet(poolName, "proverWallet", "default", null, null).get();
		Wallet proverWallet = Wallet.openWallet("proverWallet", null, null).get();
		...
		//20. Close and delete Issuer1 Wallet
		issuerGvtWallet.closeWallet().get();
		Wallet.deleteWallet("issuer1Wallet", null).get();

		//21. Close and delete Issuer2 Wallet
		issuerXyzWallet.closeWallet().get();
		Wallet.deleteWallet("issuer2Wallet", null).get();

		//22. Close and delete Prover Wallet
		proverWallet.closeWallet().get();
		Wallet.deleteWallet("proverWallet", null).get();
	...
	public void anoncredsWorksForSingleIssuerSingleProver() throws Exception {
		...
		//13. Prover create ClaimReq for GVT Claim Offer
		String xyzClaimReq = Anoncreds.proverCreateClaimReq(proverWallet, proverDid, xyzClaimOffer, xyzClaimDef, masterSecret).get();

		//14. Issuer create Claim
		String xyzClaimAttributesJson = "{\n" +
				"               \"status\":[\"partial\",\"51792877103171595686471452153480627530895\"],\n" +
				"               \"period\":[\"8\",\"8\"]\n" +
				"        }";
		...
		//19. Close and delete Issuer Wallet
		issuerWallet.closeWallet().get();
		Wallet.deleteWallet("issuerWallet", null).get();

		//21. Close and delete Their Wallet
		proverWallet.closeWallet().get();
		Wallet.deleteWallet("proverWallet", null).get();
