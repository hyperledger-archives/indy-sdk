...
static const int nTimePermitTests = 10;  /**< MPin Time Permit Tests */

int test_mpin_tp()
{
    printf("test_mpin_tp() started\n");
    ...
    for(iter=1; iter<nTimePermitTests+1; iter++)
    {
       // printf("Date %d \n", date);
        ...
        if (rtn != 0)
        {
            ...
            if (err==0)
            {
                printf("test_mpin_tp() Iteration %d FAILURE Invalid Token Error Code %d\n", iter, rtn);
                break;
            }
            else
            {
                printf("test_mpin_tp() Iteration %d FAILURE PIN Error %d, Error Code %d\n", iter, err, rtn);
                break;
            }
        }
        else
        {
            printf("test_mpin_tp() Iteration %d SUCCESS Error Code %d\n", iter, rtn);
        }
        date++;
    }
    KILL_CSPRNG(&RNG);
