...
int test_mpin_bad_token()
{
    printf("test_mpin_bad_token() started\n");
        ...
        if (err==0)
            {
                if( rtn == -19 )
                {
                    printf("test_mpin_bad_token() SUCCESS Invalid Token Error Code %d\n", rtn);
                    
                }
                else
                {
                    printf("test_mpin_bad_token() FAILURE Invalid Token Error Code %d\n", rtn);
                }
            }
        else
            printf("test_mpin_bad_token() FAILURE PIN Error %d, Error Code %d\n",err, rtn);
    }
    else
    {
        printf("test_mpin_bad_token() SUCCESS Error Code %d\n", rtn);
