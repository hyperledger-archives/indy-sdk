...
int test_mpin_bad_pin()
{
    printf("test_mpin_bad_pin() started\n");
   ...
   // OCT_output(&HCID);
    ...
    if (rtn != 0)
    {
        ...
        if (err)
            printf("test_mpin_bad_pin() SUCCESS PIN Error %d, Error Code %d\n",err, rtn);
        else
            printf("test_mpin_bad_pin() SUCCESS Invalid Token Error Code %d\n", rtn);
    }
    else
    {
        printf("test_mpin_bad_pin() SUCCESS Error Code %d\n", rtn);
