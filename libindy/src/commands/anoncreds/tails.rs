...
impl SDKTailsAccessor {
    pub async fn new(tails_service: Arc<BlobStorageService>,
               tails_reader_handle: i32,
...
impl Drop for SDKTailsAccessor {
    fn drop(&mut self) {
        #[allow(unused_must_use)] //TODO
            async {
                self.tails_service.close(self.tails_reader_handle).await
                    .map_err(map_err_err!());
            };
    }
}
        ...
        block_on(async {
            let tail_bytes = self.tails_service
                .read(self.tails_reader_handle,
                      TAIL_SIZE,
                      TAIL_SIZE * tail_id as usize + TAILS_BLOB_TAG_SZ as usize).await
                .map_err(|_|
...
pub async fn store_tails_from_generator(service: Arc<BlobStorageService>,
                                  writer_handle: i32,
    ...
    let res = service.finalize(blob_handle).await.map(|(location, hash)| (location, hash.to_base58()))?;
