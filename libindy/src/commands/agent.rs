...
//    }
    ...
    pub fn pack_msg(
        ...
        match sender {
            Some(s) => {
                //parse senders and receivers to structs
                let from_address : Result<FromAddress> = serde_json::from_str(s)
                    .map_err(|err| {
                        IndyError::AgentError(AgentError::PackError(format!("Failed to deserialize sender {}", err)))
                    });
                let to_address : Result<ToAddress> = serde_json::from_str(receivers)
                    .map_err(|err| {
                      IndyError::AgentError(AgentError::PackError(format!("Failed to deserialize receivers {}", err)))
                    });

                //encrypt sym_key for recipient
                for their_vk in receivers_list {
                    let (e_cek, cek_nonce) = self.crypto_service
                        .crypto_box(my_key, their_vk, &sym_key[..])?;

                    //serialize enc_header
                    let sender_vk_bytes = base58::decode(&my_key.verkey)
                        .map_err(|err| {
                            IndyError::CommonError(err)
                    })?;

                    //encrypt enc_from
                    let enc_sender = self.crypto_service.
                    ...
                    encrypted_recipients_struct.push(
                        Recipient {
                                encrypted_key: base58::encode(e_cek.as_bytes()),
                                header: Header {
                                    sender: Some(base64::encode(enc_sender.as_bytes())),
...
mod tests {
    ...
    pub fn test_serde_json_works() {
        let sender = r#"{ wallet_key: ["pubkey12345", wallet_handle: 1] }"#;
