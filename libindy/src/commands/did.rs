...
use crate::domain::crypto::did::{Did, DidMetadata, DidMethod, DidValue, DidWithMeta, MyDidInfo, TemporaryDid, TheirDid, TheirDidInfo};
...
pub enum DidCommand {
    ...
    KeyForDid(
        ...
        Box<dyn Fn(IndyResult<String/*key*/>) + Send + Sync>),
    KeyForLocalDid(
        WalletHandle,
        DidValue, // did (my or their)
        Box<dyn Fn(IndyResult<String/*key*/>) + Send + Sync>),
    ...
    GetEndpointForDid(
        ...
        PoolHandle, // pool handle
        DidValue, // did
        Box<dyn Fn(IndyResult<(String, Option<String>)>) + Send + Sync>),
    SetDidMetadata(
        WalletHandle,
        DidValue, // did
        String, // metadata
    ...
    QualifyDid(
        ...
        DidValue, // did
        DidMethod, // method
        Box<dyn Fn(IndyResult<String /*full qualified did*/>) + Send + Sync>,
    ),
}
...
impl DidCommandExecutor {
    ...
    pub async fn execute(&self, command: DidCommand) {
        match command {
            ...
            DidCommand::KeyForLocalDid(wallet_handle, did, cb) => {
                ...
                cb(self.key_for_local_did(wallet_handle, &did).await);
            }
            DidCommand::SetEndpointForDid(wallet_handle, did, endpoint, cb) => {
                debug!("SetEndpointForDid command received");
                cb(self.set_endpoint_for_did(wallet_handle, &did, &endpoint).await);
            }
            ...
            DidCommand::SetDidMetadata(wallet_handle, did, metadata, cb) => {
                ...
                cb(self.set_did_metadata(wallet_handle, &did, metadata).await);
            }
            DidCommand::GetDidMetadata(wallet_handle, did, cb) => {
                debug!("GetDidMetadata command received");
                cb(self.get_did_metadata(wallet_handle, &did).await);
            }
            ...
            DidCommand::QualifyDid(wallet_handle, did, method, cb) => {
                ...
                cb(self.qualify_did(wallet_handle, &did, &method).await);
            }
        };
    }
    ...
    async fn get_endpoint_for_did(&self,
                            ...
                            pool_handle: PoolHandle,
                            did: DidValue,
                            cb: Box<dyn Fn(IndyResult<(String, Option<String>)>) + Send + Sync>) {
