...
impl Validatable for CredentialDefinitionId {
    fn validate(&self) -> Result<(), String> {
        ...
        parts.get(0).ok_or(format!("Credential Definition Id validation failed: issuer DID not found in: {}", self.0))?;
        parts.get(1).ok_or(format!("Credential Definition Id validation failed: marker not found in: {}", self.0))?;
        parts.get(2).ok_or(format!("Credential Definition Id validation failed: signature type not found in: {}", self.0))?;

        if parts.len() == 4 {
            // NcYxiDXkpYi6ov5FcYDi1e:3:CL:1
            parts.get(3)
                .ok_or(format!("Credential Definition Id validation failed: schema id not found in: {}", self.0))?
                .parse::<i32>()
            ...
            parts.get(7).ok_or(format!("Credential Definition Id validation failed: tag not found in: {}", self.0))?;
        } else {
