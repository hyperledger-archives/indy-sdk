...
impl Filtering for CredentialInfo {
    ...
    fn schema_issuer_did(&self) -> String {
        self.parts().get(0).map(|s| s.to_string()).unwrap_or(String::new())
    }
    fn schema_name(&self) -> String {
        self.parts().get(2).map(|s| s.to_string()).unwrap_or(String::new())
    }
    fn schema_version(&self) -> String {
        self.parts().get(3).map(|s| s.to_string()).unwrap_or(String::new())
    }
