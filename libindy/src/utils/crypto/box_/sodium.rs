...
mod tests {
    ...
    fn encrypt_decrypt_works() {
        ...
        let seed = sign::Seed::from_slice(&randombytes::randombytes(32)).unwrap();

        let (alice_ver_key, alice_sign_key) = CryptoSign::create_key_pair_for_signature(Some(&seed)).unwrap();
        let alice_pk = CryptoSign::vk_to_curve25519(&alice_ver_key).unwrap();
        let alice_sk = CryptoSign::sk_to_curve25519(&alice_sign_key).unwrap();

        let (bob_ver_key, bob_sign_key) = CryptoSign::create_key_pair_for_signature(Some(&seed)).unwrap();
        let bob_pk = CryptoSign::vk_to_curve25519(&bob_ver_key).unwrap();
        let bob_sk = CryptoSign::sk_to_curve25519(&bob_sign_key).unwrap();
