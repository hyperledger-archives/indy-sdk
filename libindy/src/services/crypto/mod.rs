...
impl CryptoService {
        ...
        if !self.is_crypto_type_supported(crypto_type_name) {
            return Err(
                CryptoError::UnknownCryptoError(
                    format!("contains unknown crypto type: {}", crypto_type_name)));
        }
    ...
    pub fn encrypt(&self, my_key: &Key, their_vk: &str, doc: &[u8]) -> Result<(Vec<u8>, Vec<u8>), CryptoError> {
        let (_, crypto_type_name) = CryptoService::get_verkey_and_crypto_type_name_from_verkey(my_key.verkey.as_str());
    ...
    pub fn deserialize_seed(seed: Option<&str>) -> Result<Option<Vec<u8>>, CryptoError> {
        // TODO: Fixme: This looks like its checking for seed to be base64 or otherwise and convert
