...
impl Verifier {
    ...
    pub fn new_nonce(&self) -> Result<Nonce, CommonError> {
        verifier::Verifier::new_nonce()
    ...
    pub fn verify(&self,
        ...
        for (claim_id, claim_definition) in claim_defs {
            ...
            let attrs_for_claim = Verifier::_get_revealed_attributes_for_claim(claim_id.as_str(), &full_proof.requested_proof, proof_req)?;
            let predicates_for_claim = Verifier::_get_predicates_for_claim(claim_id.as_str(), &full_proof.requested_proof, proof_req)?;
            ...
            proof_verifier.add_sub_proof_request(claim_id.as_str(),
                                                 &sub_proof_request,
                                                 &claim_schema,
    ...
    fn _get_revealed_attributes_for_claim(claim_id: &str, requested_proof: &RequestedProof, proof_req: &ProofRequest) -> Result<Vec<String>, CommonError> {
        info!(target: "services/anoncreds/verifier", "_get_revealed_attributes_for_claim >>> claim_id: {:?}, requested_claims: {:?}, proof_req: {:?}",
              claim_id, requested_proof, proof_req);

        let mut revealed_attrs_for_claim: Vec<String> = Vec::new();

        for (attr_uuid, &(ref requested_claim_id, _, _)) in &requested_proof.revealed_attrs {
            if claim_id.eq(requested_claim_id.as_str()) {
    ...
    fn _get_predicates_for_claim(claim_id: &str, requested_proof: &RequestedProof, proof_req: &ProofRequest) -> Result<Vec<PredicateInfo>, CommonError> {
        info!(target: "services/anoncreds/verifier", "_get_predicates_for_claim >>> claim_id: {:?}, requested_claims: {:?}, proof_req: {:?}",
              claim_id, requested_proof, proof_req);

        let mut predicates_for_claim: Vec<PredicateInfo> = Vec::new();

        for (predicate_uuid, requested_claim_id) in &requested_proof.predicates {
