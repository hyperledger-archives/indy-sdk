...
fn anoncreds_demo_works() {
    ...
    let err =
        indy_create_wallet(issuer_create_wallet_command_handle,
    ...
    // Issuer Opens Wallet
    let err =
        indy_open_wallet(issuer_open_wallet_command_handle,
                         CString::new(issuer_wallet_config.as_str()).unwrap().as_ptr(),
                         CString::new(issuer_wallet_credentials.as_str()).unwrap().as_ptr(),
    ...
    // Prover Opens Wallet
    let err =
        indy_open_wallet(prover_open_wallet_command_handle,
                         CString::new(prover_wallet_config.as_str()).unwrap().as_ptr(),
                         CString::new(prover_wallet_credentials.as_str()).unwrap().as_ptr(),
    ...
    let res = indy_close_wallet(issuer_close_wallet_command_handle, issuer_wallet_handle, issuer_close_wallet_callback);
    ...
    let res = indy_close_wallet(prover_close_wallet_command_handle, prover_wallet_handle, prover_close_wallet_callback);
    assert_eq!(res, ErrorCode::Success);
...
fn ledger_demo_works() {
    ...
    let err =
        indy_build_nym_request(build_nym_request_command_handle,
    ...
    let err =
        indy_build_get_nym_request(build_get_nym_request_command_handle,
                                   CString::new(my_did.as_str()).unwrap().as_ptr(),
                                   CString::new(my_did.as_str()).unwrap().as_ptr(),
