...
use utils::anoncreds::{COMMON_MASTER_SECRET, CLAIM1_ID, CLAIM2_ID};
...
//                                                                      None).unwrap();
//
//            AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM1_ID, &claim_json, None, None).unwrap();
//        }
//
//        #[test]
...
//
//            let invalid_wallet_handle = wallet_handle + 100;
//            let res = AnoncredsUtils::prover_store_claim(invalid_wallet_handle, CLAIM1_ID, &claim_json, None, None);
//            assert_eq!(res.unwrap_err(), ErrorCode::WalletInvalidHandle);
//        }
//    }
//
...
//
//            let res = AnoncredsUtils::prover_store_claim(wallet_handle, CLAIM1_ID, &claim_json, None, None);
//            assert_eq!(res.unwrap_err(), ErrorCode::WalletNotFoundError);
//        }
//
//        #[test]
...
mod demos {
    ...
    //        AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM1_ID, &serde_json::to_string(&claim_json).unwrap(), None, None).unwrap();
    //
    ...
    //        AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM1_ID, &xclaim_json, None, None).unwrap();
    //
    ...
    fn anoncreds_works_for_single_issuer_single_prover() {
        ...
        let claim_def_json = AnoncredsUtils::issuer_create_claim_definition(issuer_wallet_handle, ISSUER_DID, &schema_json, "TAG1", None, false).unwrap();
        let claim_def: serde_json::Value = serde_json::from_str(&claim_def_json).unwrap();
        let cred_def_id = claim_def["id"].as_str().unwrap();

        //4. Prover creates Master Secret
        AnoncredsUtils::prover_create_master_secret(prover_wallet_handle, COMMON_MASTER_SECRET).unwrap();

        //5. Issuer creates Credential Offer
        let claim_offer_json = AnoncredsUtils::issuer_create_claim_offer(issuer_wallet_handle, &cred_def_id, None, DID_MY1).unwrap();
        ...
        let requested_claims_json = format!(r#"{{
                                                  "self_attested_attributes":{{"attr3_referent":"{}"}},
                                                  "requested_attrs":{{
                                                        "attr1_referent":[{{"cred_id":"{}"}}, true],
                                                        "attr2_referent":[{{"cred_id":"{}"}}, false]
                                                  }},
                                                  "requested_predicates":{{
        ...
        let schemas_json = format!(r#"{{"{}":{}}}"#, proof.requested_proof.unrevealed_attrs["attr2_referent"], schema_json);
        let claim_defs_json = format!(r#"{{"{}":{}}}"#, proof.requested_proof.unrevealed_attrs["attr2_referent"], claim_def_json);
        let rev_reg_defs_json = "{}";
        let rev_regs_json = "{}";
...
//        //3. Issuer creates claim definition
//        let schema_json = AnoncredsUtils::gvt_schema_json();
//        let claim_def_json = AnoncredsUtils::issuer_create_claim_definition(issuer_wallet_handle,
//                                                                            ISSUER_DID,
//                                                                            &schema_json,
//                                                                            "claim_def1",
//                                                                            None,
//                                                                            true).unwrap();
//        let claim_def: serde_json::Value = serde_json::from_str(&claim_def).unwrap();
//
//
...
//        AnoncredsUtils::store_witness(prover_wallet_handle, CLAIM1_ID, &witness_json).unwrap();
//
//        //12. Prover store received Credential
//        AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM1_ID, &claim_json, Some(&revoc_reg_def_json), Some(&revoc_reg_delta_json)).unwrap();
//
//        //13. Prover gets Claims for Proof Request
//        let claims_json = AnoncredsUtils::prover_get_claims_for_proof_req(prover_wallet_handle, &AnoncredsUtils::proof_request_attr()).unwrap();
//        let claim = AnoncredsUtils::get_claim_for_attr_referent(&claims_json, "attr1_referent");
//
...
//        let schemas_json = format!(r#"{{"{}":{}}}"#, claim.referent, schema_json);
//        let claim_defs_json = format!(r#"{{"{}":{}}}"#, claim.referent, claim_def_json);
//        let rev_reg_defs_json = format!("{{\"{}\":{}}}", claim.referent, revoc_reg_def_json);
//        let rev_reg_entries_json = format!("{{\"{}\":{}}}", claim.referent, revoc_reg_delta_json);
//
//        let proof_json = AnoncredsUtils::prover_create_proof(prover_wallet_handle,
//                                                             &AnoncredsUtils::proof_request_attr(),
//                                                             &requested_claims_json,
...
//    }
    ...
    //        AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM1_ID, &gvt_claim_json, None, None).unwrap();
    //
    ...
    //        AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM2_ID, &xyz_claim_json, None, None).unwrap();
    //
    ...
    //        //12. Prover store received Credential
    //        AnoncredsUtils::prover_store_claim(prover_wallet_handle, CLAIM1_ID, &claim_json, Some(&revoc_reg_def_json), Some(&revoc_reg_delta_json)).unwrap();
    //        //13. Issuer revokes claim
    //        let revoc_reg_delta_json = AnoncredsUtils::issuer_revoke_claim(issuer_wallet_handle, tails_reader_handle, &ISSUER_DID,
    ...
    //        let schemas_json = format!(r#"{{"{}":{}}}"#, claim.referent, schema_json);
    //        let claim_defs_json = format!(r#"{{"{}":{}}}"#, claim.referent, claim_def_json);
    //        let rev_reg_defs_json = format!(r#"{{"{}":{}}}"#, claim.referent, revoc_reg_def_json);
    //        let rev_reg_entries_json = format!(r#"{{"{}":{}}}"#, claim.referent, revoc_reg_delta_json);
    //
    //        let proof_json = AnoncredsUtils::prover_create_proof(prover_wallet_handle,
    //                                                             &AnoncredsUtils::proof_request_attr(),
    //                                                             &requested_claims_json,
    ...
    //        let revoc_reg_delta_json = AnoncredsUtils::issuer_revoke_claim(issuer_wallet_handle, tails_reader_handle, &ISSUER_DID, &schema_json, SEQ_NO as u32).unwrap();
    //
    //        let rev_reg_entries_json = format!("{{\"{}\":{}}}", claim.referent, revoc_reg_delta_json);
    //
