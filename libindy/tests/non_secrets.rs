...
mod high_cases {
    ...
    mod add_record {
        ...
        fn indy_add_wallet_record_works_for_empty_params() {
            ...
            assert_eq!(ErrorCode::CommonInvalidParam3, res.unwrap_err().error_code);

            let res = add_wallet_record(wallet_handle, TYPE, "", VALUE, None);
            assert_eq!(ErrorCode::CommonInvalidParam4, res.unwrap_err().error_code);

            let res = add_wallet_record(wallet_handle, TYPE, ID, "", None);
            assert_eq!(ErrorCode::CommonInvalidParam5, res.unwrap_err().error_code);
