...
describe('call to cxs_connection_connect ', function() {
    var path = parentDir.dirname(currentDir);
    path += "/lib/libcxs.so";
    var run = new CXSRuntime(new CXSRuntimeConfig(path));
    it('should return 0', function () {
        assert.equal(run.ffi.cxs_connection_connect(2), 0)
    })
});
...
describe('call to cxs_connection_get_data ', function() {
    ...
    var run = new CXSRuntime(new CXSRuntimeConfig(path));
    it('should return 0', function () {
        assert.equal(run.ffi.cxs_connection_get_data(2, "get_data"), 0)
    })
});
...
describe('call to cxs_connection_get_state ', function() {
    ...
    var run = new CXSRuntime(new CXSRuntimeConfig(path));
    it('should return 0', function () {
        assert.equal(run.ffi.cxs_connection_get_state(2, 'state'), 0)
    })
});
