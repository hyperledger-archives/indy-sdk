...
// parse from a json that has been generated by a non-encrypted query "to_string()"
pub fn parse_from_json(json: &str) -> Result<Operator, WalletQueryError> {
    parse_from_json_iternal(json, false)
}

// parse from a json that has been generated by an encrypted query "to_string()"
pub fn parse_from_json_encrypted(json: &str) -> Result<Operator, WalletQueryError> {
    parse_from_json_iternal(json, true)
}

fn parse_from_json_iternal(json: &str, encrypted: bool) -> Result<Operator, WalletQueryError> {
...
fn parse(map: serde_json::Map<String, serde_json::Value>, encrypted: bool) -> Result<Operator, WalletQueryError> {
    let mut operators: Vec<Operator> = Vec::new();

    for (key, value) in map.into_iter() {
        let suboperator = parse_operator(key, value, encrypted)?;
...
fn parse_operator(key: String, value: serde_json::Value, encrypted: bool) -> Result<Operator, WalletQueryError> {
    match (&*key, value) {
        ("$and", serde_json::Value::Array(values)) => {
            ...
            for value in values.into_iter() {
                if let serde_json::Value::Object(map) = value {
                    let suboperator = parse(map, encrypted)?;
                    operators.push(suboperator);
                } else {
        ...
        ("$not", serde_json::Value::Object(map)) => {
            let operator = parse(map, encrypted)?;
        ...
        (_, serde_json::Value::String(value)) => {
            ...
            Ok(Operator::Eq(target_name, target_value))
        },
        (_, serde_json::Value::Object(map)) => {
            if map.len() == 1 {
                let (operator_name, value) = map.into_iter().next().unwrap();
                parse_single_operator(operator_name, key, value, encrypted)
            } else {
...
fn parse_single_operator(operator_name: String, key: String, value: serde_json::Value, encrypted: bool) -> Result<Operator, WalletQueryError> {
    // if "encrypted" then the keys and values have already been base64'ed
    match (&*operator_name, value) {
        ("$neq", serde_json::Value::String(s)) => {
            let target_name = _build_target_name(&key, encrypted);
            let target_value = _build_target_value(&target_name, &s, encrypted);
            Ok(Operator::Neq(target_name, target_value))
        },
        ("$neq", _) => Err(WalletQueryError::ValueErr("$neq must be used with string".to_string())),
        ("$gt", serde_json::Value::String(s)) => {
            let target_name = _build_target_name(&key, encrypted);
            match target_name {
                TagName::PlainTagName(_) => {
                    let target_value = _build_target_value(&target_name, &s, encrypted);
                    Ok(Operator::Gt(target_name, target_value))
                },
        ...
        ("$gte", serde_json::Value::String(s)) => {
            ...
            match target_name {
                TagName::PlainTagName(_) => {
                    let target_value = _build_target_value(&target_name, &s, encrypted);
                    Ok(Operator::Gte(target_name, target_value))
                },
        ...
        ("$lt", serde_json::Value::String(s)) => {
            ...
            match target_name {
                TagName::PlainTagName(_) => {
                    let target_value = _build_target_value(&target_name, &s, encrypted);
                    Ok(Operator::Lt(target_name, target_value))
                },
        ...
        ("$lte", serde_json::Value::String(s)) => {
            ...
            match target_name {
                TagName::PlainTagName(_) => {
                    let target_value = _build_target_value(&target_name, &s, encrypted);
                    Ok(Operator::Lte(target_name, target_value))
                },
        ...
        ("$like", serde_json::Value::String(s)) => {
            ...
            match target_name {
                TagName::PlainTagName(_) => {
                    let target_value = _build_target_value(&target_name, &s, encrypted);
                    Ok(Operator::Like(target_name, target_value))
                },
        ...
        ("$in", serde_json::Value::Array(values)) => {
            ...
            Ok(Operator::In(target_name, target_values))
        }
...
fn _build_target_name(key: &String, encrypted: bool) -> TagName {
    if !encrypted {
        TagName::from(key.to_string()).unwrap()
    } else {
        let key_int;
        if key.starts_with("~") {
            key_int = base64::decode(&key[1..]).unwrap();
            TagName::PlainTagName(key_int)
        } else {
            key_int = base64::decode(&key).unwrap();
            TagName::EncryptedTagName(key_int)
        }
    }
}

fn _build_target_value(target_name: &TagName, value: &String, encrypted: bool) -> TargetValue {
    match (encrypted, target_name) {
        (true, TagName::EncryptedTagName(_)) => TargetValue::Encrypted(base64::decode(value).unwrap()),
        (_, _) => TargetValue::from(value.to_string())
    }
}
