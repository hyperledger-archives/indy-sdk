...
impl Prover {
    ...
    pub fn prepare_requested_proof_response(&self, proof_req: &ProofRequestJson, requested_claims: &RequestedClaimsJson,
        ...
        let mut self_attested: HashMap<String, String> = HashMap::new();
    ...
    pub fn create_proof(&self,
        ...
        for (proof_claim_uuid, proof_claim) in &proof_claims {
            ...
            if let Some(ref non_revocation_claim) = proof_claim.claim_json.signature.non_revocation_claim {
                let proof = Prover::_init_non_revocation_proof(non_revocation_claim,
                                                               &proof_claim.revocation_registry.accumulator,
                                                               &proof_claim.claim_definition.public_key_revocation,
                                                               tails)?;
...
mod tests {
    ...
    fn finalize_proof_works() {
        ...
        let res = Prover::_finalize_proof(&ms, &proof, &c_h, &encoded_attributes, &revealed_attributes, &unrevealed_attributes);
...
pub mod mocks {
    ...
    pub fn get_gvt_claims_object() -> Result<Claims, CryptoError> {
        Ok(Claims::new(get_gvt_primary_claim()?, None))
    }

    pub fn get_xyz_claims_object() -> Result<Claims, CryptoError> {
        Ok(Claims::new(get_xyz_primary_claim()?, None))
    }

    pub fn get_all_claims() -> Result<HashMap<SchemaKey, (Claims, HashMap<String, BigNumber>)>, CryptoError> {
        let mut res: HashMap<SchemaKey, (Claims, HashMap<String, BigNumber>)> = HashMap::new();
        res.insert(get_gvt_schema_key(), (get_gvt_claims_object()?, ::services::crypto::anoncreds::issuer::mocks::get_gvt_encoded_attributes()?));
        Ok(res)
    }

    pub fn get_all_claims_2_rows() -> Result<HashMap<SchemaKey, (Claims, HashMap<String, BigNumber>)>, CryptoError> {
        let mut res: HashMap<SchemaKey, (Claims, HashMap<String, BigNumber>)> = HashMap::new();
        res.insert(get_gvt_schema_key(), (get_gvt_claims_object()?, ::services::crypto::anoncreds::issuer::mocks::get_gvt_encoded_attributes()?));
        res.insert(get_xyz_schema_key(), (get_xyz_claims_object()?, ::services::crypto::anoncreds::issuer::mocks::get_xyz_encoded_attributes()?));
        Ok(res)
    }
    ...
    pub fn get_accumulator() -> Result<Accumulator, CryptoError> {
        ...
        v.insert(1);
        Ok(Accumulator::new(PointG1::new().unwrap(), v, 5, 2))
    }
