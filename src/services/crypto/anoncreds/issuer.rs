...
mod tests {
    ...
    fn generate_keys_works() {
        ...
        let claim_definition_result = ClaimDefinition::new(public_key, None, 1, "CL".to_string());
        let claim_definition_private_result = ClaimDefinitionPrivate::new(secret_key, None);
        let (claim_definition, claim_definition_private) = issuer.generate_keys(get_gvt_schema(), None, false).unwrap();
        assert_eq!(claim_definition, claim_definition_result);
        assert_eq!(claim_definition_private, claim_definition_private_result);
    }
