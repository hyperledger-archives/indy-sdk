...
impl UpdateMessages{
    ...
    pub fn send_secure(&mut self) -> Result<(), u32> {
        ...
        println!("update_message content: {:?}", data);
...
fn parse_update_messages_response(response: Vec<u8>) -> Result<(), u32> {
    ...
    let response: UpdateMessages = match Deserialize::deserialize(&mut de) {
        Ok(x) => x,
        Err(x) => {
...
pub fn update_agency_messages(status_code: &str, msg_json: &str) -> Result<(), u32> {
    ...
    let mut messages = UpdateMessages {
        msg_type: MsgType { name: "UPDATE_MSG_STATUS_BY_CONNS".to_string(), ver: "1.1".to_string(), },
        uids_by_conns,
