...
impl ProofMessage {
    ...
    pub fn get_credential_info(&self) -> Result<Vec<CredentialData>, u32> {
        self.proofs.iter().map(|(credential_uuid, proof_data)|
                Ok(CredentialData::create(Some(proof_data.schema_seq_no),
    ...
    fn get_revealed_attrs(&self) -> Result<Vec<CredentialData>, u32> {
        ...
        self.requested_proof.revealed_attrs.iter().map(|(attr_id, attr_data)| {
            ...
            Ok(CredentialData::create( Some(credential_info.schema_seq_no),
                               Some(credential_info.issuer_did.to_string()),
                               Some(credential_uuid.to_string()),
...
impl EqAndGeProof {
    ...
    pub fn get_predicates_from_credential(&self, uuid: &str) -> Result<Vec<CredentialData>, u32> {
        self.ge_proofs.iter().map(|ge_proof| {
            ...
            Ok(CredentialData::create(predicate.schema_seq_no,
...
pub mod tests {
    ...
    fn test_get_credential_data() {
        ...
        assert_eq!(credential_data[0].credential_uuid.clone().unwrap(), "claim::71b6070f-14ba-45fa-876d-1fe8491fe5d4");
        assert_eq!(credential_data[0].issuer_did.clone().unwrap(), "V4SGRU86Z58d6TV7PBUe6f".to_string());
        assert_eq!(credential_data[0].schema_seq_no.clone().unwrap(), 103);
    }

    #[test]
    fn test_get_proof_attrs_multiple_credentials() {
        let add_credential: Proofs = serde_json::from_str(r#"{"proof":{"primary_proof":{"eq_proof":{"revealed_attrs":{"t2":"Hash for 2"},"a_prime":"3","e":"2","v":"5","m":{"t2":"2"},"m1":"2","m2":"2"},"ge_proofs":[{"u":{"2":"2","0":"2","3":"2","1":"2"},"r":{"1":"2","3":"2","DELTA":"2","2":"2","0":"2"},"mj":"3","alpha":"2","t":{"0":"2","2":"2","DELTA":"4","1":"5","3":"3"},"predicate":{"attr_name":"predicate2","p_type":"LE","value":99,"schema_seq_no":778,"issuer_did":"12345"}}]},"non_revoc_proof":null},"schema_seq_no":778,"issuer_did":"12345"}"#).unwrap();
        ...
        assert!(attrs_str.contains(r#"{"schema_seq_no":103,"issuer_did":"V4SGRU86Z58d6TV7PBUe6f","credential_uuid":"claim::71b6070f-14ba-45fa-876d-1fe8491fe5d4","attr_info":{"name":"name","value":"Alex","type":"revealed"}}"#));
        assert!(attrs_str.contains(r#"{"schema_seq_no":778,"issuer_did":"12345","credential_uuid":"claim2_uuid","attr_info":{"name":"t2","value":"t2_val","type":"revealed"}}"#));

        //Check predicates
        assert!(attrs_str.contains(r#"{"schema_seq_no":778,"issuer_did":"12345","credential_uuid":"claim2_uuid","attr_info":{"name":"predicate2","value":99,"type":"predicate","predicate_type":"LE"}}"#));
        assert!(attrs_str.contains(r#"{"schema_seq_no":14,"issuer_did":"33UDR9R7fjwELRvH9JT6HH","credential_uuid":"claim::71b6070f-14ba-45fa-876d-1fe8491fe5d4","attr_info":{"name":"age","value":18,"type":"predicate","predicate_type":"GE"}}"#));
