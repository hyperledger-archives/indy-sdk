...
if [ "$1" = "--help" ] ; then
  echo "Usage: <architecture> <version> <key> <number> <artifact_name> <is_stable>"
  return
fi
...
EOF
    ...
    ssh -v -oStrictHostKeyChecking=no -i $key repo@192.168.11.115 mkdir -p /var/repository/repos/android/${artifact}/rc/${version}-${buildNumber}
