...
impl CreateClaimDef {
    ...
    pub fn build_create_txn(&self, claim_def_json: &str) -> Result<String, CredDefError> {
        libindy_build_create_claim_def_txn(&self.claim_def.issuer_did,
                                           self.claim_def.schema_seq_no as i32,
                                           Some(SigTypes::CL),
                                           claim_def_json).map_err(|ec| CredDefError::CommonError(ec))
    }
...
impl ClaimDefinition {
    ...
    fn from_str(claim_def: &str) -> Result<Self, CredDefError> {
...
pub mod tests {
    ...
    fn test_get_claim_def_by_send_request_fails() {
        settings::set_defaults();
        assert_eq!(RetrieveClaimDef::new().send_request("{}"), Err(CredDefError::CommonError(error::NO_POOL_OPEN.code_num)));
    }

    #[test]
    fn test_build_claim_def_req_fails() {
        assert_eq!(RetrieveClaimDef::new().build_get_txn("", 0, None, ""),
    ...
    fn test_create_claim_def_fails_with_already_created_claim_def() {
        ...
        let (my_did, _) = SignusUtils::create_and_store_my_did(wallet_handle, Some(DEMO_ISSUER_PW_SEED)).unwrap();
        SignusUtils::create_and_store_my_did(wallet_handle, Some(DEMO_AGENT_PW_SEED)).unwrap();
        assert_eq!(create_new_claimdef("1".to_string(), "name".to_string(), 263, my_did, false).err(),
