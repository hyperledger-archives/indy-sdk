...
pub fn prep_anonymous_msg(recipient_vk: &str, msg: &[u8]) -> VcxResult<Vec<u8>> {
    if settings::mock_indy_test_mode_enabled() { return Ok(Vec::from(msg).to_owned()); }
...
pub fn parse_msg(recipient_vk: &str, msg: &[u8]) -> VcxResult<(String, Vec<u8>)> {
    if settings::mock_indy_test_mode_enabled() { return Ok((::utils::constants::VERKEY.to_string(), Vec::from(msg).to_owned())); }
...
pub fn pack_message(sender_vk: Option<&str>, receiver_keys: &str, msg: &[u8]) -> VcxResult<Vec<u8>> {
    if settings::mock_indy_test_mode_enabled() { return Ok(msg.to_vec()); }
