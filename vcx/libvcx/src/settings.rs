...
impl From<String> for ProtocolTypes {
    fn from(type_: String) -> Self {
        match type_.as_str() {
            "1.0" => ProtocolTypes::V0,
            "2.0" => ProtocolTypes::V1,
            type_ @ _ => {
                error!("Unknown protocol type: {:?}. Community use as default", type_);
...
impl ::std::string::ToString for ProtocolTypes {
    fn to_string(&self) -> String {
        match self {
            ProtocolTypes::V0 => "1.0".to_string(),
            ProtocolTypes::V1 => "2.0".to_string(),
        }
    }
}
