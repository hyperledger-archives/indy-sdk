...
impl AgentConnection {
    ...
    pub(super) fn handle_create_general_message(&mut self,
    ...
    pub(super) fn get_messages(&self, msg: GetMessages) -> Vec<GetMessagesDetailResponse> {
    ...
    pub(super) fn handle_agent2conn_message(&mut self,
    ...
    pub(super) fn create_and_store_internal_message(&mut self,
                                                    uid: Option<&str>,
                                                    mtype: RemoteMessageType,
    ...
    pub(super) fn is_sent_by_owner(&self, sender_verkey: &str) -> bool {
        sender_verkey == self.user_pairwise_verkey
    }

    pub(super) fn is_sent_by_remote(&self, sender_verkey: &str) -> bool {
    ...
    pub(super) fn get_message_handler_role(&self, sender_verkey: &str) -> MessageHandlerRole {
    ...
    pub(super) fn validate_general_message(&self, reply_to_msg_id: Option<&str>) -> Result<(), Error> {
    ...
    pub(super) fn answer_message(&mut self, uid: &str, ref_msg_id: &str, status_code: &MessageStatusCode) -> Result<(), Error> {
    ...
    pub(super) fn store_their_did(&self, did: &str, verkey: &str) -> ResponseFuture<(), Error> {
    ...
    pub(super) fn check_if_message_not_already_answered(&self, status_code: &MessageStatusCode) -> Result<(), Error> {
    ...
    pub(super) fn check_valid_status_code(&self, status_code: &MessageStatusCode) -> Result<(), Error> {
    ...
    pub(super) fn check_no_connection_established(&self) -> Result<(), Error> {
    ...
    pub(super) fn check_no_accepted_invitation_exists(&self) -> Result<(), Error> {
    ...
    pub(super) fn verify_agent_key_dlg_proof(&self, sender_verkey: &str, key_dlg_proof: &KeyDlgProof) -> ResponseFuture<(), Error> {
    ...
    pub(super) fn send_remote_message(&self, message: Vec<u8>, endpoint: String) -> ResponseFuture<(), Error> {
        let router = self.router.clone();
        future::ok(())
            .and_then(move |_| {
                router.read().unwrap()
                    .route_to_requester(RemoteMsg { endpoint, body: message })
                    .from_err()
    ...
    pub(super) fn get_remote_endpoint(&self) -> Result<String, Error> {
    ...
    pub(super) fn prepare_remote_message(&self, message: Vec<A2AMessage>) -> Result<Vec<u8>, Error> {
    ...
    pub(super) fn build_payload_message<T>(&self, type_: RemoteMessageType, msg: &T) -> ResponseFuture<Vec<u8>, Error> where T: ::serde::Serialize + ::std::fmt::Debug {
    ...
    pub(super) fn build_forward_message(&self, fwd: &str, message: Vec<u8>) -> Result<Vec<A2AMessage>, Error> {
